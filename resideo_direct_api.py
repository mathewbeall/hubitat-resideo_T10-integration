...
class ResideoAPIController:
    def __init__(self, client_id: str, client_secret: str, redirect_uri: str = None):
        ...
        self.redirect_uri = redirect_uri or "http://localhost:8090/callback"

        # Resideo API base URLs - migrated from api.honeywell.com (cert expired May 2026)
        self.auth_base_url = "https://api.honeywellhome.com/oauth2"
        self.api_base_url = "https://api.honeywellhome.com/v2"

        self.access_token = None
        self.refresh_token = None
        self.token_expires = None
        self.last_refresh_time = None  # Track when we last refreshed to avoid rapid refreshes
    ...
    def _ensure_valid_token(self) -> bool:
        ...
        # Check if we just refreshed within the last 60 seconds (cooldown)
        if self.last_refresh_time:
            seconds_since_refresh = (datetime.now() - self.last_refresh_time).total_seconds()
            if seconds_since_refresh < 60:
                return True  # Recently refreshed, consider token fresh

        # Calculate time remaining on token
        if self.token_expires:
            time_remaining = self.token_expires - datetime.now()
            minutes_remaining = int(time_remaining.total_seconds() / 60)

            # If token has 30+ minutes remaining, it was JUST refreshed - consider it fresh
            if minutes_remaining >= 30:
                return True

            # Proactively refresh tokens with < 30 min remaining
            # Resideo invalidates tokens server-side aggressively
            print(f"🔄 Proactively refreshing Resideo token ({minutes_remaining} min remaining)")
    ...
    def _make_api_request(self, method: str, endpoint: str, data: Dict = None) -> Dict:
        ...
        try:
            ...
            if hasattr(e, 'response') and e.response is not None:
                # Check for 401 Unauthorized - token may be revoked/invalid
                if e.response.status_code == 401:
                    print(f"⚠️ 401 Unauthorized (fallback handler triggered - proactive refresh may need tuning)")

                    # Try to refresh the token
                    refresh_result = self.refresh_access_token()

                    if refresh_result.get('success'):
                        print(f"✅ Fallback token refresh successful, retrying request...")
                        # Retry the request with new token
                        return self._make_api_request(method, endpoint, data)
                    else:
                        print(f"❌ Fallback token refresh failed: {refresh_result.get('error')}")
                        return {"error": f"Token refresh failed: {refresh_result.get('error')}"}
    ...
    def get_all_devices(self) -> Dict:
        ...
        for location in locations:
            ...
            for device in devices:
                device['_locationID'] = location_id
    ...
    def get_thermostat_priority(self, device_id: str, location_id: int) -> Dict:
        """
        Get room priority settings for a specific thermostat
        ...
        Returns:
            Dict: Priority settings including priorityType and selectedRooms
        """
        return self._make_api_request('GET', f'/devices/thermostats/{device_id}/priority?locationId={location_id}')

    def set_thermostat_priority(self, device_id: str, location_id: int, priority_type: str, selected_rooms: List[int] = None) -> Dict:
        """
        Set room priority for a specific thermostat

        Args:
            device_id (str): Thermostat device ID
            location_id (int): Location ID where thermostat is located
            priority_type (str): Priority mode - "PickARoom", "FollowMe", or "WholeHouse"
            selected_rooms (List[int]): Room IDs to use (only for PickARoom mode)
        ...
        # Build priority setting based on type
        priority_setting = {'priorityType': priority_type}

        if priority_type == 'PickARoom':
            if not selected_rooms:
                return {"error": "PickARoom mode requires selected_rooms list"}
            priority_setting['selectedRooms'] = selected_rooms

        return self._make_api_request('PUT', f'/devices/thermostats/{device_id}/priority?locationId={location_id}', {
            'currentPriority': priority_setting
        })

    def get_thermostat_rooms(self, device_id: str, location_id: int, group_id: int = 0) -> Dict:
        """
        Get detailed room and sensor information for a thermostat

        Args:
            device_id (str): Thermostat device ID
            location_id (int): Location ID where thermostat is located
            group_id (int): Group ID (usually 0)

        Returns:
            Dict: Room details including sensors, temperature, humidity, occupancy
        """
        return self._make_api_request('GET', f'/devices/thermostats/{device_id}/group/{group_id}/rooms?locationId={location_id}')
